...
class CloudStorageURLSigner(object):
  ...
  def Put(self, path, content_type, data):
    ...
    Returns:
      An instance of requests.Response containing the HTTP response.
    """
    md5_digest = base64.b64encode(hashlib.md5(data.encode()).digest()).decode("utf-8") 
...
#     headers['Content-Length'] = str(len(data))
  ...
  def GeneratePutUrl(self, path, content_type, data_md5):
    base_url, query_params = self._MakeUrl('PUT', path, content_type,
                                           data_md5)
    headers = {}
    headers['Content-Type'] = content_type
...
def main():
  ...
  print('Creating file...')
  print('================')
  r = signer.Put(file_path, 'text/plain', 'Hello World!')
  ProcessResponse(r)
